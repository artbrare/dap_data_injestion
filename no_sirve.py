...
class CommodityTicketsProcessor:
    ...
    def update_database_tables(self, conn, staging_data: Dict[str, List[Dict[str, Any]]]):
        """Actualiza las tablas de la base de datos usando inserción en paralelo"""
        try:
            # Mapeo de nombres de tablas
            table_mapping = {
                'commodity_tickets': 'commodity_tickets',
                'applications': 'commodity_tickets_applications',
                'discounts': 'commodity_tickets_discounts',
                'grade_factors': 'commodity_tickets_grade_factors'
            }

            for table_name, data in staging_data.items():
                if not data and table_name == 'commodity_tickets':
                    continue

                actual_table = table_mapping[table_name]
                self.logger.info(f"Insertando {len(data)} registros en {actual_table}")
                
                records_inserted = self.bulk_insert_data(conn, actual_table, data)
                self.logger.info(f"Insertados {records_inserted} registros en {actual_table}")
    ...
    def bulk_insert_data(self, conn, table_name: str, data: List[Dict[str, Any]], max_workers: int = 12):
        if not data:
            return
        ...
        # Convertir a DataFrame y filtrar columnas
        ...
        def process_chunk(chunk):
            ...
            with self.get_connection() as conn:
                ...
                insert_sql = f"INSERT INTO {temp_table} ({','.join(chunk.columns)}) VALUES ({placeholders})"
